...
- [x] ログ設定の追加
- [x] 音声入力機能（Speech-to-Text）（2025-12-20追加）
  - GCP Speech-to-Text APIを使用した音声テキスト変換
  - チャット画面での音声録音・変換機能
...
8. **POST /api/speech/transcribe/** - 音声テキスト変換（2025-12-20追加）
   - 音声ファイルをテキストに変換
   - GCP Speech-to-Text API使用
...
- ✅ バリデーション強化済み
- ✅ Django Admin改善済み
- ✅ フロントエンドダッシュボード機能実装済み
- ✅ 音声入力機能実装済み（GCP Speech-to-Text API）

### フロントエンド機能

#### ダッシュボード（2024/12/02追加）
- **アクセス**: サイドバーのダッシュボードアイコン（グラフアイコン）からアクセス
- **統計カード表示**:
  - 総セッション数
  - 平均スコア（完了セッションの平均）
  - 最高スコア
- **直近のセッション履歴**: 最新5件のセッションを表示
  - 日付、業界、モード（簡易/詳細）、スコアを表示
  - クリックでセッション再開可能
- **設定エリア**: ペルソナ設定などユーザー個別設定機能の追加予定エリア（Coming Soon）
- **データ取得**: `/api/session/list/` APIを使用

#### 音声入力機能（2025/12/20追加）
- **機能**: チャット画面で音声を録音し、テキストに自動変換
- **技術**: GCP Speech-to-Text APIを使用
- **UI**: マイクボタンで録音開始/停止、録音時間表示
- **API**: `POST /api/speech/transcribe/` - 音声ファイルをテキストに変換
- **詳細**: `AIdocs/SPEECH_TO_TEXT_FEATURE.md` を参照
