...
def _generate_customer_response_legacy(session, conversation_history):
    ...
    # 挨拶フェーズ用の強化されたプロンプト
    greeting_phase_prompt = """
=== 【挨拶フェーズ - 絶対に守ること】 ===
あなたは営業担当者から訪問を受けた顧客です。
営業担当者が挨拶をしてきたので、挨拶を返すだけにしてください。

【必ず守ること】
- 挨拶のみを返す（1文だけ）
- 返答例：「はい、よろしくお願いします。」「こんにちは。」

【絶対に禁止 - 以下は絶対にしないこと】
- 質問をすること（「何か〜」「どのような〜」「教えてください」「お聞かせください」）
- 説明をすること（「まずは〜」「要点を絞って〜」「お話を伺います」）
- 「御社の〜」と相手の会社について質問すること
- 2文以上の返答
...
【間違った返答例（絶対にやらないこと）】
「こちらこそよろしくお願いします。まずは簡単にお話伺います。」← 禁止
「よろしくお願いします。御社の課題について教えていただけますか？」← 禁止
"""

    if question_count == 0:
        conversation_phase_instruction = greeting_phase_prompt
...
def generate_customer_response_stream(session, conversation_history):
    ...
    # 挨拶フェーズ用の強化されたプロンプト（ストリーミング版）
    greeting_phase_prompt_stream = """
=== 【挨拶フェーズ - 絶対に守ること】 ===
あなたは営業担当者から訪問を受けた顧客です。
営業担当者が挨拶をしてきたので、挨拶を返すだけにしてください。
...
"""
    ...
    if question_count == 0:
        conversation_phase_instruction = greeting_phase_prompt_stream
    elif question_count <= 2:
        conversation_phase_instruction = (
            "- 営業担当者の質問に丁寧に答えますが、まだ慎重な姿勢を保ち、"
            "必要な範囲で情報を提供する程度にとどめてください。回答は2〜3文程度で。"
...
def _build_system_prompt(session, conversation_history) -> str:
    ...
    # 挨拶フェーズ用の強化されたプロンプト（_build_system_prompt版）
    greeting_phase_prompt_build = """
=== 【挨拶フェーズ - 絶対に守ること】 ===
あなたは営業担当者から訪問を受けた顧客です。
営業担当者が挨拶をしてきたので、挨拶を返すだけにしてください。
...
"""
    ...
    if question_count == 0:
        conversation_phase_instruction = greeting_phase_prompt_build
    elif question_count <= 2:
        conversation_phase_instruction = (
            "- 営業担当者の質問に丁寧に答えますが、まだ慎重な姿勢を保ち、"
            "必要な範囲で情報を提供する程度にとどめてください。回答は2〜3文程度で。"
